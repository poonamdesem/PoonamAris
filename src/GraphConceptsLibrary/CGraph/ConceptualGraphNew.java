...
import core.AbstractSyntaxTreeNew;
import core.ConceptualGraphBuilder;
//import core.NodeTraversal;

public class ConceptualGraphNew extends
	...
	private ConceptualGraphNew graph;
	...
	public ConceptualGraphNew(GraphEdgeFactory edgeFactory) {
		super(edgeFactory);
		this.edgeFactory = edgeFactory;
	}
    public double ConvergeValue;
    public GraphConcept Root;
    public GraphConcept getRoot() {
		return Root;
	}

	public void setRoot(GraphConcept root) {
		Root = root;
	}
   ...
   /* public boolean addVertex(GraphConcept concept)
    ...
    public static ConceptualGraphNew loadGraphFromStringNew(String code)
    {
        AbstractSyntaxTreeNew tree = new AbstractSyntaxTreeNew();
        tree.loadASTFromInputTextStringNew(code);
        edgeFactory = new GraphEdgeFactory();
        ConceptualGraphNew graph = new ConceptualGraphNew(edgeFactory);
       graph.loadGraphFromASTNew(tree);
	     System.out.println("graph=="+graph);

        return graph;
    }
    private void loadGraphFromASTNew(AbstractSyntaxTreeNew ast) {
 	     System.out.println("ast=="+ast);
 	     ASTNode root = ast.getTree().getRoot();
 	     System.out.println("root=="+root);
 		rootBlock = new BlockSyntax(ast.getFileName(), mainMethod);
        this.addVertex(rootBlock);
		Object[] decs = getBodyDeclarations(root);
    
		System.out.println("decs====================="+root+"\n");
	...
	/*public void loadGraphFromAST(AbstractSyntaxTreeNew ast)
    {
	...
	private Method createMethodFromMethodDeclaration(MethodDeclaration node) {
		System.out.println("method block=="+node);
		...
		if (!node.isConstructor()) {
			...
			if (block != null && block.statements().size() != 0) {
				methodBlock = createBlockFromBlockSyntax(block,
						method.getName());
				System.out.println("method name=="+methodBlock);
